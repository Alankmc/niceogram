...
function Board() {
  this.initialize = (updateToolCallback) => {
    ...
    background = new Background(
      colorController.getColor("BACKGROUND_COLOR"),
      this.canvas.width,
      this.canvas.height,
      this.c,
    );
  ...
  this.initEventListeners = (updateToolCallback) => {
    this.canvas.onmousedown = () => {
      if (this.chosenCell >= 0 && chosenTool !== tickType.BLANK) {
        ...
        ticks[this.chosenCell] =
          chosenTool === tickType.DELETE ? tickType.BLANK : chosenTool;
        isPainting = true;
        if (chosenTool !== tickType.X) {
          updateDirectionTextColor(
            this.chosenCell,
            updateDirectionHelpers(this.chosenCell),
          );
        }
        this.checkWin();
      }
    ...
    this.canvas.onmousemove = (e) => {
      ...
      leftBoard =
        mouseX > xCheatSheet[xCheatSheet.length - 1] + positions.CELL_SIZE ||
        mouseX < xMapStart ||
      ...
      if (lastChosen !== this.chosenCell) {
        ...
        lastChosen >= 0 &&
          yDirectionHighlight[Math.floor(lastChosen / NUM_Y)].setActive(false);
        this.chosenCell >= 0 &&
          yDirectionHighlight[Math.floor(this.chosenCell / NUM_Y)].setActive(
            true,
          );
        lastChosen >= 0 &&
          xDirectionHighlight[lastChosen % NUM_Y].setActive(false);
        this.chosenCell >= 0 &&
          xDirectionHighlight[this.chosenCell % NUM_Y].setActive(true);
        ...
        if (
          isPainting &&
          this.chosenCell >= 0 &&
          cells[this.chosenCell].getTick() !== paintingAs
        ) {
          cells[this.chosenCell].paintCell(chosenTool);
  ...
  this.animate = () => {
    ...
    background.update();
    cells.forEach((el) =>
      el.update(mouseX, mouseY, this.chosenCell, leftBoard, this),
    );
    // toolText.update();
  ...
  this.boardInit = () => {
    ...
    for (let i = 0; i < NUM_Y; i++) {
      ...
      xDirectionHighlight.push(
        new DirectionHighlight(
          positions.X_START,
          yMapStart +
            i * positions.CELL_SIZE +
            (i - 1) * positions.GAP_SIZE +
            temp * positions.GAP_5_SIZE,
      ...
      for (let j = 0; j < currDirection.length; j++) {
        currLine.push(
          new Text(
            xMapStart -
              (j + 1) * (positions.CELL_SIZE + DIRECTION_GAP) +
              DIRECTION_TEXT_PAD,
            yMapStart +
              i * (positions.CELL_SIZE + positions.GAP_SIZE) +
              temp * positions.GAP_5_SIZE +
              DIRECTION_TEXT_PAD,
        ...
        cells.push(
          new Cell(
            xMapStart +
              i * positions.CELL_SIZE +
              (i - 1) * positions.GAP_SIZE +
              x5gaps * positions.GAP_5_SIZE,
            yMapStart +
              j * positions.CELL_SIZE +
              (j - 1) * positions.GAP_SIZE +
              y5gaps * positions.GAP_5_SIZE,
            index++,
            colorController,
            this.c,
          ),
...
function randomWin(tickedPercentage) {
  for (let i = 0; i < NUM_X; i++) {
    for (let j = 0; j < NUM_Y; j++) {
      win.push(
        Math.random() < tickedPercentage ? tickType.TICKED : tickType.BLANK,
      );
    }
  }
}
...
function getTrains(tickArray, coord, isX, len) {
  ...
  for (let i = 0; i < len; i++) {
    nextCoord = isX ? NUM_Y * i + coord : i + coord * NUM_X;
...
function compareTrainArrays(expected, current) {
  ...
  if (
    ...
    expected.reduce((acc, el) => (el > acc ? el : acc), 0) <
      current.reduce((acc, el) => (el > acc ? el : acc), 0)
  ) {
...
function buildDirections() {
  ...
  xMapStart =
    positions.X_START + (positions.CELL_SIZE + DIRECTION_GAP) * maxXdirections;
  yMapStart =
    positions.Y_START + (positions.CELL_SIZE + DIRECTION_GAP) * maxYdirections;
}
...
function getCellByPosition(x, y) {
  ...
  if (
    x > xCheatSheet[NUM_X - 1] + positions.CELL_SIZE ||
    y > yCheatSheet[NUM_Y - 1] + positions.CELL_SIZE
  ) {
    return -2;
  }

  const foundX = xCheatSheet.findIndex(
    (el, ind) => ind === NUM_X || (x >= el && x < xCheatSheet[ind + 1]),
  );
  const foundY = yCheatSheet.findIndex(
    (el, ind) => ind === NUM_Y || (y >= el && y < yCheatSheet[ind + 1]),
  );
...
function updateDirectionTextColor(chosenCell, newHelpers) {
  ...
  for (var i = 0; i < newHelpers.x.length; i++) {
    if (newHelpers.x[i]) {
      xDirectionText[y][newHelpers.x.length - i - 1].setColor(
        colorController.getColor("COLOR_CORRECT_DIRECTION_TEXT"),
      );
    } else {
      xDirectionText[y][newHelpers.x.length - i - 1].setColor(
        colorController.getColor("TEXT_COLOR"),
  ...
  for (var i = 0; i < newHelpers.y.length; i++) {
    if (newHelpers.y[i]) {
      yDirectionText[x][newHelpers.y.length - i - 1].setColor(
        colorController.getColor("COLOR_CORRECT_DIRECTION_TEXT"),
      );
    } else {
      yDirectionText[x][newHelpers.y.length - i - 1].setColor(
